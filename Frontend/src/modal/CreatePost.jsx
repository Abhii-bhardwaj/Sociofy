...
const handlePostSubmit = async () => {
  ...
  formData.append("caption", caption || ""); // Latest caption
  console.log("FormData caption:", formData.get("caption")); // Debug
  ...
  try {
    ...
    if (isEditMode) {
      ...
      const response = await axiosInstance.put(
        `/post/${postToEdit._id}`, // Ensure route matches backend
        formData,
        {
      ...
      console.log("Edit API Response:", response.data); // ✅ Response check
      await editPost(postToEdit._id, formData); // This might be redundant—remove if not needed
      await fetchPosts();
      ...
      <div className="bg-[#1a1f35] bg-opacity-90 rounded-xl shadow-lg p-6 w-[90%] sm:w-[75%] md:max-w-lg border border-white transition-all duration-300 scale-100">
        ...
        <textarea
          ...
          onChange={(e) => {
            ...
            console.log("Caption changed to:", e.target.value); // ✅ Yeh check karo
          }}
